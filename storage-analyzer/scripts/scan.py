...
def scandir_children(path, min_kb=51200, limit=80):
...
def scan_windows():
    ...
    local_low = os.path.join(os.path.dirname(local), "LocalLow")
    targets = [
        ("user_profile", profile, 102400),
        ("appdata_local", local, 10240),
        ("appdata_roaming", roaming, 10240),
        ("temp", os.environ.get("TEMP", os.path.join(local, "Temp")), 10240),
        ("downloads", os.path.join(profile, "Downloads"), 10240),
        ...
        # 扩展扫描：系统级目录（只读，仅大小统计）
        ("program_data", "C:\\ProgramData", 10240),
        ("appdata_low", local_low, 10240),
        ("packages_store", os.path.join(local, "Packages"), 10240),
        ("inet_cache", os.path.join(local, "Microsoft", "Windows", "INetCache"), 10240),
        ("windows_temp", os.environ.get("SystemRoot", "C:\\Windows") + "\\Temp", 10240),
        ("windows_software_distribution", os.environ.get("SystemRoot", "C:\\Windows") + "\\SoftwareDistribution", 10240),
        ("intel", "C:\\Intel", 51200),
        ("perflogs", "C:\\PerfLogs", 51200),
        ("inetpub", "C:\\inetpub", 51200),
    ]
    groups = {}
    ...
    dev_paths = [
        ...
        os.path.join(local, "Microsoft", "VSCommon"),       # VS shared
        os.path.join(local, "Microsoft", "dotnet"),          # .NET SDK
        # Wine / Docker for Windows
        os.path.join(profile, ".wine"),
        # Rustup
        os.path.join(profile, ".rustup"),
        # Scoop / Chocolatey package managers
        os.path.join(local, "scoop"),
        os.path.join(profile, "scoop"),
