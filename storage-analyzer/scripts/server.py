...
def _trash_windows(path):
    # Shell32 SHFileOperationW is deprecated on Windows 10 19045+ and returns
    # ERROR_CALL_NOT_IMPLEMENTED (120) for most directories. Use PowerShell's
    # Microsoft.VisualBasic.FileSystem which wraps IFileOperation — the current
    # Microsoft-recommended API for sending to Recycle Bin.
    try:
        ...
        if r.returncode != 0:
            ...
            if "not supported on this system" in err_lower or "不支持该功能" in err_msg:
                raise OSError("系统保留目录，需管理员权限才能移到回收站（可右键「以管理员身份运行」本终端再试）")
            elif "system call level is not correct" in err_lower:
                raise OSError("目标正在被占用（如浏览器缓存需先关闭浏览器再试）")
            elif "denied" in err_lower or "permission" in err_lower or "access" in err_lower:
                raise OSError("权限不足，请以管理员身份运行再试")
            else:
                raise OSError("回收站操作失败：%s" % err_msg)
    except subprocess.TimeoutExpired:
        raise OSError("回收站操作超时")
    except OSError:
        raise
    except Exception as e:
        raise OSError("移到回收站失败：%s" % e)
    ...
    def do_POST(self):
        ...
        for p in (req.get("paths") or []):
            ...
            if rp not in allow:
                ...
                return
            # 二级护栏：只允许用户目录、/Applications（macOS）或系统程序目录（Windows）
            # 绿色项的白名单已确保路径安全，此护栏为防御纵深——防止意外路径穿越
